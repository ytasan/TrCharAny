...
- [ ] **Python version:** README says Python **3.10+**; the real requirement in [`pyproject.toml`](pyproject.toml) is **`requires-python = ">=3.11"`** (because `turkish-deasciifier` from Git targets 3.11+). Update README to **3.11+** and match wording with install/run instructions.
